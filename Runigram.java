...
public class Runigram {
	...
	public static void main(String[] args) {
		...
		print(imageOut);
		System.out.println();
		flippedVert = flippedVertically (tinypic);
		print(flippedVert);
		System.out.println();
		print(grayScaled(tinypic));
		System.out.println();
		scaledVers = scaled (tinypic, 3, 5);
		print(scaledVers);
		*/
	...
	public static Color luminance(Color pixel) {
		...
		Color grayPix = new Color(lum , lum , lum);
		return grayPix;
	}
	...
	public static Color[][] grayScaled(Color[][] image) {
		Color [][] grayVers = new Color [image.length][image[0].length];
		for (int i = 0; i < grayVers.length; i++) {
			for (int j = 0; j < grayVers[i].length; j++) {
				grayVers[i][j] = luminance(image[i][j]);
			}
		}
		return grayVers;
	}	
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				int rowScaled = i * image.length / height;
				int colScaled = j * image[0].length / width;
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		int blendedR = (int) (c1.getRed() * alpha + c2.getRed() * (1-alpha));
		int blendedG = (int) (c1.getGreen() * alpha + c2.getGreen() * (1-alpha));
		int blendedB = (int) (c1.getBlue() * alpha + c2.getBlue() * (1-alpha));
